...
> Tugas Kelas B — Sistem Komputasi Terdistribusi — 4 Kelompok Bekerja Sama, 1 Jaringan Bersama
...
### Data Kelompok Kelas B

| Kelompok | Wilayah | Universitas | Repository Sumber Data | PIC (Penerima Akses VM) |
|----------|---------|-------------|------------------------|-------------------------|
| **Kelompok 1** | Jawa Tengah | Universitas Diponegoro (Undip) | https://eprints2.undip.ac.id/ | **Syifani Adillah Salsabila** |
| **Kelompok 2** | Jawa Barat | IPB University (IPB) | https://repository.ipb.ac.id/ | **Muhammad Rafly Ash Shiddiqi** |
| **Kelompok 3** | Jawa Timur | Universitas Brawijaya (UB) | https://repository.ub.ac.id/ | **Asyraf Rizqy K** |
| **Kelompok 4** | Sulawesi Selatan | Universitas Hasanuddin (Unhas) | http://repository.unhas.ac.id/cgi/oai2 | **M. Wahyu Lillah** |
...
|----------|----|-------------|-----------------------|------------------------|
| **K1** – Syifani | VM-1 | Undip | Buat & distribusikan `genesis.json`, koordinasi setup awal | Jalankan node, submit skripsi dari repo Undip, ikut konsensus |
| **K2** – Rafly | VM-2 | IPB | Konfigurasi dan deploy Smart Contract ke jaringan | Jalankan node, submit skripsi dari repo IPB, ikut konsensus |
| **K3** – Asyraf | VM-3 | UB | Bangun REST API & plagiarism engine (dapat dipakai semua) | Jalankan node, submit skripsi dari repo UB, ikut konsensus |
| **K4** – Wahyu | VM-4 | Unhas | Implementasi OAI-PMH Crawler + IPFS Storage Layer (pipeline otomatis: harvest via OAI-PMH → parse Dublin Core XML → upload ke IPFS → register on-chain) | Jalankan node, submit skripsi dari repo Unhas, ikut konsensus |
...
Tahap 4 — Operasional & Pengujian
  ...
  └── K4 demo pipeline: OAI-PMH harvest → parse XML → upload IPFS → register blockchain
```
...
| Sinkronisasi genesis.json | K1 (Syifani) upload ke repo bersama (Git/shared drive) |
| Contract address | K2 (Rafly) umumkan setelah deploy berhasil |
| Troubleshooting | Cek log geth: `geth --verbosity 3`, bandingkan antar kelompok |
| Sumber data scraping | Ambil dari repository masing-masing: Undip / IPB / UB / Unhas (K4 via OAI-PMH) |
...
│          Kelas B — SKT — Private Consortium Network                 │
│                                                                     │
│   ┌──────────────┐              ┌──────────────┐                    │
│   │  VM-1        │◄────────────►│  VM-2        │                    │
│   │  K1 – Tel-U  │              │  K2 – UB     │                    │
